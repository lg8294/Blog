...
<feed xmlns="http://www.w3.org/2005/Atom">
  ...
  <link rel="self" type="application/atom+xml" href="{{ feed_url | uriencode }}"/>
  {% if config.feed.hub %}<link href="{{ config.feed.hub | uriencode }}" rel="hub"/>{% endif %}
  <link rel="alternate" type="text/html" href="{{ url | uriencode }}"/>
  ...
  <entry>
    ...
    <link rel="alternate" type="text/html" href="{{ (url + post.path) | uriencode }}"/>
    <id>{{ (url + post.path) | uriencode }}</id>
    {% if config.author %}
    <author>
